...
class iCRDecoder(nn.Module):
    def __init__(self, d_model: int, hidden_dim: int, output_dim: int, nlayers: int, dropout: float):
        super().__init__()
...
class Attention(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        energy = torch.tanh(self.attn(hidden + encoder_outputs))
...
class iCRDecoderWithAttention(nn.Module):
    def __init__(self, d_model: int, hidden_dim: int, output_dim: int, nlayers: int, dropout: float):
        super().__init__()
        ...
        # LSTM layer to decode text embeddings
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True, dropout=dropout)

        # Attention mechanism
        self.attention = Attention(hidden_size)

        # Linear layers to map LSTM output and context to text embeddings
    ...
    def forward(self, inputs, hidden, encoder_outputs):
        ...
        # Attention mechanism
        attn_weights = self.attention(lstm_out, encoder_outputs)
        context = torch.bmm(attn_weights.unsqueeze(1), encoder_outputs)
        
        # Concatenate LSTM output and context
        lstm_out = lstm_out.squeeze(1)
        context = context.squeeze(1)
        output = torch.cat((lstm_out, context), 1)
        
        # Linear layer to get text embeddings
        text_embeddings = self.fc(output)

        return text_embeddings, hidden, attn_weights
